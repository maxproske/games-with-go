...
// NoiseType indicates which noise type to generate
type NoiseType int
...
func MakeNoise(noiseType NoiseType, frequency, lacunarity, gain float32, octaves, w, h int) (noise []float32, min, max float32) {
	...
	for i := 0; i < numRoutines; i++ {
		go func(i int) {
			...
			for j := start; j < end; j++ {
				...
				// Check if we have noise below the min or above the max
				if noise[j] < innerMin {
					innerMin = noise[j]
			...
			// Only lock once per goroutine, not milliosn of times inside the loop.
			// Check if inner min is less than min
			// mutex.Lock()
			// if innerMin < min {
			// 	min = innerMin
	...
	// Close our channel
	close(minMaxChan)
	// Now iterate over it (same thing as mutexes)
...
// Snoise2 generates 2D simplex noise
