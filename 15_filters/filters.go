...
// Not a balloon receiever function, so each balloon isn'tt updating itself
func updateBalloons(balloons []*balloon, elapsedTime float32, currentMouseState mouseState, prevMouseState mouseState, audioState *audioState) []*balloon {

	numAnimations := 16
	balloonClicked := false
	balloonsExploded := false

	// Sorting ensures closest balloon is drawn last
	for i := len(balloons) - 1; i >= 0; i-- {
		balloon := balloons[i]

		// If the balloon is exploding, check to see if it is done
		if balloon.exploding {
	...
	// Remove a baloon from the list of it has exploded. Shrink the list.
	if balloonsExploded {
		// Reuse the same chunk of memory, replacing popped balloons with unpopped balloons (cool!)
		filteredBalloons := balloons[0:0] // point to the same memory start of slice
		for _, balloon := range balloons {
			if !balloon.exploded {
