...
// Take in a string and return a node
func stringToNode(s string) Node {
	...
	case "Picture":
		return NewOpPicture()
	default:
		panic("Error in stringToNode. Token not understood: " + s)
	}
}

// Recursive so we can descend into the tree
func parse(tokens chan token, parent Node) Node {
	for {
		...
		// Do something based on what token we get
		switch token.typ {
		case op:
			n := stringToNode(token.value)
			n.SetParent(parent)
			for i := range n.GetChildren() {
				n.GetChildren()[i] = parse(tokens, n) // Go through each child, and set it to the parse of the rest of the tokens
			}
			return n // Exit infinite loop
		case constant:
			n := NewOpConstant()
			n.SetParent(parent)
			// Check for invalid characters
			v, err := strconv.ParseFloat(token.value, 32) // 32-bit float
			if err != nil {
				panic("Error while parsing constant op: " + token.value)
			}
			n.value = float32(v)
			return n // Constants are always leaves, return immediately
		case closeParen, openParen:
			continue // We don't even need to know about them. All ops have a known number of children
		}
	}
...
func BeginLexing(s string) Node {
	...
	return parse(l.tokens, nil) // Parser takes tokens. nil becuase top of treee doesn't have a parent to start recursion
}
