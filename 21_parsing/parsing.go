...
import (
	...
	. "github.com/PrawnSkunk/games-with-go/21_parsing/apt"
	. "github.com/PrawnSkunk/games-with-go/21_parsing/gui"
...
func main() {
	...
	if len(args) > 1 { // First argument is always program's name itself
		...
		pictureNode := BeginLexing(fileStr) // Begin lexing our string
		// Make picture struct
		...
		state.zoom = true // Make full screen
		state.zoomImage = tex
		state.zoomTree = p
	}
